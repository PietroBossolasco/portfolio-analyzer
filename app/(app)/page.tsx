  ...
  return (
    <div className="space-y-6">
      <header className="relative overflow-hidden rounded-2xl bg-hero-gradient p-6 text-white shadow-lift md:p-8">
        ...
        <div className="relative flex flex-wrap items-end justify-between gap-4">
          <div>
            ...
            <div className="mt-1 font-display text-4xl font-extrabold tabular-nums">{fmtEur(s.patrimonio)}</div>
      ...
      <section className="grid grid-cols-2 gap-4 lg:grid-cols-4">
        <Kpi label="Valore portafoglio" value={fmtEur(s.patrimonio)} sub="ultimo estratto PDF" accent />
        <Kpi label="Capitale conferito" value={fmtEur(s.capitaleNetto)} sub="versamenti netti alla data PDF" />
        <Kpi
        ...
        />
        <Kpi label="Liquidità" value={fmtEur(s.liquidita)} />
        <Kpi label="Dividendi incassati" value={fmtEur(s.dividendi)} sub="storico" tone="pos" />
        <Kpi label="Interessi/cedole" value={fmtEur(s.interessi)} sub="storico" tone="pos" />
        <Kpi
      ...
      <section className="grid grid-cols-2 gap-4 text-sm lg:grid-cols-4">
        <MiniStat label="Commissioni pagate" value={fmtEur(s.commissioni)} />
        <MiniStat label="Imposte/bolli" value={fmtEur(s.tasse)} />
        <MiniStat label="Reddito passivo netto" value={fmtEur(s.redditoPassivoNetto)} />
...
function MiniStat({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-slate-200/80 bg-white px-4 py-3 shadow-card">
      ...
      <div className="font-display font-semibold text-ink-900">{value}</div>
    </div>
  );
}
